...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        # dimension: [batch_size, heads, max_len, d_k]
        Q, K, V = [l(x).view(nbatches, -1, self.h, self.d_k).transpose(1, 2)
                   for l, x in zip(self.linears, (query, key, value))]

        x, self.attn = attention(Q, K, V, mask=mask, dropout=self.dropout)

        x = x.transpose(1, 2).contiguous().view(nbatches, -1, self.h * self.d_k) # [batch_size, max_len, d_model]
...
def make_src_mask(n_pts: int) -> torch.Tensor:
    """Return a (1,1,n_pts) all-True mask for GraFormer attention."""
    return torch.ones(1, 1, n_pts, dtype=torch.bool)
...
class GraFormer(nn.Module):
    def __init__(self, adj, hid_dim=128, in_feat=256, out_dim=3, num_layers=4,
        ...
        # Register adj as a buffer — .to(device) will move it automatically
        self.register_buffer('adj', adj)

        _gconv_input = ChebConv(in_c=in_feat, out_c=hid_dim, K=2)
        _gconv_layers = []
        _attention_layer = []
        ...
        self.register_buffer('src_mask', make_src_mask(n_pts))

        for i in range(num_layers):
            # IMPORTANT: pass adj=None here — _ResChebGC will use self.adj buffer
            # after the model is moved to device.  Passing the CPU adj tensor here
            # and registering it again in _ResChebGC causes a device mismatch when
            # GraFormer.to(cuda) moves GraFormer.adj but _ResChebGC.adj stays on CPU.
            _gconv_layers.append(_ResChebGC(adj=adj, input_dim=hid_dim, output_dim=hid_dim,
    ...
    def forward(self, x):
        # self.adj and self.src_mask are registered buffers — always on correct device
...
if __name__ == '__main__':
    ...
    model = GraFormer(adj=adj, hid_dim=128, n_pts=21)
    x = torch.zeros((1, 21, 2))
    print(model(x))  # forward takes only x; src_mask is internal
